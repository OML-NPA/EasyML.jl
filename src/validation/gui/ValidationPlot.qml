...
ApplicationWindow {
    ...
    function load_model_classes(classModel) {
        ...
        if (problem_type==0 || problem_type==1) {
            ...
            for (var i=0;i<num_classes;i++) {
                ...
                var class_var = {
                    "name": Julia.get_class_field(ind,"name"),
                    "overlap": Julia.get_class_field(ind,"overlap"),
                    "border": Julia.get_class_field(ind,["BorderClass","enabled"])}
                classModel.append(class_var)
            }
        }
    }
    ...
    property int input_type: Julia.get_input_type()
    property int problem_type: Julia.get_problem_type()
    ...
    Component.onCompleted: {
        use_labels = Julia.get_data(["ValidationData","PlotData","use_labels"])
        if (problem_type==0) {
    ...
    onClosing: {
        Julia.put_channel("validation_modifiers",[0.0,0.0])
    }
    ...
    Timer {
        ...
        property string channel_name: "validation_start"
        property bool grabDone: false
        onTriggered: {
            while (true) {
                var state = Julia.get_progress(channel_name)
                if (state===false) {
                    return
                }
                if (max_iterations===-1) {
                    ...
                    if (state===false) {
                        ...
                        channel_name = "validation_progress"
                    }
                }
                else if (iteration<max_iterations) {
                    iteration += 1
                    validationProgressBar.value = iteration/max_iterations
                    var accuracy_temp = state[0]
                    var loss_temp = state[1]
                    ...
                    if (iteration==1) {
                        ...
                        classComboBox.currentIndex = 0
                        var ind1 = 1
                        var size = Julia.get_image_size(["ValidationData",fieldname,"original_images"],[ind1])
                        var s = 1024*pix
                        ...
                        displayItem.scale = r
                        if (problem_type==0) {
                            Julia.get_image_validation(["ImageClassificationResults","original_images"],[ind1])
                            ...
                            Julia.get_image_validation(["ImageRegressionResults","original_images"],[ind1])
            ...
            Column {
                ...
                Row {
                    ...
                    StopButton {
                        ...
                        width: buttonHeight
                        height: buttonHeight
                        onClicked: Julia.put_channel("validation_modifiers",[0.0,0.0])
                    }
                }
                Label {
                    visible: use_labels
                ...
                Row {
                    ...
                    SpinBox {
                        ...
                        onValueModified: {
                            ...
                            var size = Julia.get_image_size(["ValidationData",fieldname,"original_images"],[ind1+1])
                            var s = 1024*pix
                            var r = Math.min(s/size[0],s/size[1])
                            displayItem.height = size[0]*r
                            displayItem.width = size[1]*r
                            ...
                            displayItem.image_width = size[1]
                            displayItem.scale = r
                            Julia.get_image_validation([fieldname,"original_images"],[ind1+1])
                            originalDisplay.update()
                            if (problem_type==0) {
                                if (use_labels) {
                            ...
                            if (problem_type==1) {
                                ...
                                Julia.get_image_validation(["ImageSegmentationResults",typeComboBox.type],[ind1+1,ind2+1])
                                resultDisplay.visible = true
                                resultDisplay.update()
                            }
                        }
                ...
                Row {
                    ...
                    ComboBox {
                        ...
                        onActivated: {
                            var ind1 = sampleSpinBox.value
                            var ind2 = classComboBox.currentIndex
                            Julia.get_image_validation(["ImageSegmentationResults",typeComboBox.type],[ind1,ind2+1])
                            resultDisplay.update()
                        }
                        Component.onCompleted: {
                            for (var i=0;i<classModel.count;i++) {
                ...
                Row {
                    ...
                    ComboBox {
                        ...
                        onActivated: {
                            ...
                            var ind1 = sampleSpinBox.value
                            var ind2 = classComboBox.currentIndex
                            Julia.get_image_validation(["ImageSegmentationResults",type],[ind1,ind2+1])
                            resultDisplay.update()
                        }
                    }
                }
